...
function magnetTearsMod:onDamage(entity, amt, flag, source, countdown)
  ...
  if (tear.Type == EntityType.ENTITY_TEAR or tear.Type == EntityType.ENTITY_LASER) and tear:GetData().Magnet then
    
      entity:GetData().Magneted = true
      --entity:AttachSprite()...
	  entity:SetColor(WhiteColor ,0 ,0, false, false)
      entity:GetData().MagnetTimer = 5
    end
	
  else
    for i=1, # entities do
      local laser = entities[i]
      if laser.Type == EntityType.ENTITY_LASER and laser:GetData().Magnet then
        entity:GetData().Magneted = true
...
magnetTears:AddCallback( ModCallbacks.MC_NPC_UPDATE, magnetTears.Behaviour, bhEntity); 
magnetTears:AddCallback( ModCallbacks.MC_POST_UPDATE, magnetTears.Update);
